...
if (strpos( $params->get ('moduleclass_sfx'), 'cols')!==false){
	...
	foreach ($class_sfx as $clsSfx){
		if (strpos( $clsSfx, 'cols')!==false){
			...
			$pwidth = ' span' . floor (OGRID_SYS / $cols);
			// $ipercol = ceil(count($items)/$cols);
			$ipercol = floor(count($items)/$cols);
			$surplus = count($items)%$cols;
		}
	}
}
// var_dump($cols.' '.$pwidth.' '.$ipercol.' '.count($items)%$cols);
?>
				...
				<?php foreach ($items as $key=>$item):	?>
					<div class="k2ItemBlock">
						<div class="k2ItemBlock-i">
							...
							<?php if($params->get('itemImage') && isset($item->image) && ($params->get('itemDateCreated') || $params->get('itemHits'))): ?>
								<div class="moduleItemImageInfo">
							<?php endif; ?>
									<?php if($params->get('itemImage') && isset($item->image)): ?>
										...
										<div class="clr"></div>
									<?php endif; ?>
									<?php if($params->get('itemDateCreated') || $params->get('itemHits')): ?>
										<div class="moduleItemInfo">
											<?php if($params->get('itemDateCreated')): ?>
												<span class="moduleItemDateCreated">
													<?php // echo JHTML::_('date', $item->created, JText::_('K2_DATE_FORMAT_LC')); ?>
													<?php echo JHTML::_('date', $item->created, JText::_('F jS Y')); ?>
												</span>
											<?php endif; ?>
									...
									<?php endif; ?>
							<?php if($params->get('itemImage') && isset($item->image) && $params->get('itemDateCreated')): ?>
								</div>
							<?php endif; ?>
							<div class="clr"></div>
							...
							<?php if($params->get('itemTags') && count($item->tags)>0): ?>
								<div class="moduleItemTags">
									...
									<?php foreach ($item->tags as $i=>$tag): ?>
										<a href="<?php echo $tag->link; ?>"><?php echo $tag->name; ?></a><?php echo $i+1 == count($item->tags) ? '' : ', '; ?>
									<?php endforeach; ?>
								</div>
							<?php endif; ?>
					...
					<div class="clr"></div>
					<?php if ($surplus==0) { 
						if (($key+1)%$ipercol==false && $key+1 < count($items)) {
							echo '</div><div class="'.$pwidth.'">';
						}
					} else {
						if (($key+1)%($ipercol+1)==false && $key+1 < count($items)) {
							echo '</div><div class="'.$pwidth.'">';
							$surplus--;
						}
					} ?>
