...
$img = $imagebutton ? $img ? $img : 'templates/ot_clinical/images/search_btn.png' : 'templates/ot_clinical/images/search_btn.png';
$output = '';
$cls = '';
    ...
    <form action="<?php echo JRoute::_('index.php');?>" method="post" class="form-search">
		<div class="input-append">
    		<?php
				...
				if (trim($params->get('label', '')) != '') $output .= '<label for="mod-search-searchword" class="element-invisible">' . $label . '</label>';
				...
				if ($button) :
					if ($imagebutton) :
						...
						$button = ' <input type="image" alt="' . $button_text . '" class="button" src="' . $img . '" onclick="this.form.searchword.focus();"/>';
					else :
						// modify to check if no button text set in backend, then display the search icon from bootstrap
						if (trim($params->get('button_text', '')) == '') $button_text = '<span class="icon-search icon-white"></span>';
