...
?>
<div class="latestnews style3">
	<?php foreach ($list as $key=>$item) : ?>
		<?php 
		if ($key==0) : ?>
			<div class="item-leading<?php echo count($list)>1 ? ' span6' : ''; ?>">
			...
			<?php echo count($list)>1 ? '<div class="span6"><ul class="list-items">' : ''; ?>
		<?php else : ?>
			<li>
			...
			<?php echo (count($list)>1 && $key==count($list)-1) ? '</ul></div>' : ''; ?>
		<?php endif ?>
	<?php endforeach; ?>
</div>
