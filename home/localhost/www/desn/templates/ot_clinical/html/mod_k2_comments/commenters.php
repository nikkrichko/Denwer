	...
	<div id="k2ModuleBox<?php echo $module->id; ?>" class="k2TopCommentersBlock<?php echo $slide; ?>">
		<ul class="itemsList<?php echo $inner; ?>">
			<?php foreach ($commenters as $key=>$commenter): ?>
				<li class="item<?php echo count($comments)==$key+1 ? ' lastItem' : ''; echo $key==0 ? ' firstItem' : ''; echo (strpos( $params->get ('moduleclass_sfx'), 'slide') && $key==0) ? " active" : ""; ?>">

					<span class="ico-bq-o"></span>
		...
		<?php if (strpos( $params->get ('moduleclass_sfx'), 'slide')!==false){?>
			<div class="ot-slidenav">
				...
				<a class="carousel-control control-square control-mini left" href="#k2ModuleBox<?php echo $module->id; ?>" data-slide="prev"><span class="icon-prev"></span></a>
				<!-- "next page" action -->
				<a class="carousel-control control-square control-mini right" href="#k2ModuleBox<?php echo $module->id; ?>" data-slide="next"><span class="icon-next"></span></a>
			</div>
		<?php } ?>
	</div>
<?php endif; ?>
